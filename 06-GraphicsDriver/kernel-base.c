...
int start(){
    // Initialize the text mode memory address for standard VGA 80x25 mode
    TM_START = (char*) 0xb8000;
    CELL = 0; // Reset cursor to the top-left corner

    cls();                 // Clear any existing text on the screen
    setMonitorColor(0x0a); // Set the default background to black and text to light green

    // Define standard boot strings
    char WELCOME[] = "Welcome to the 4M1N-OS :) !!!\n";
    char CLI_VERSION[] = "Command Line Version 1.0.0\n\n";
    char PROMPT[] = "4M1N-OS > ";

    // Output the strings to the screen
    printString(WELCOME);
    printString(CLI_VERSION);
    printColorString(PROMPT , 0x0a); // Enforce light green specifically for the prompt
}
